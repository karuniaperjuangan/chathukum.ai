...
export default function ChatbotPage() {
  ...
  const [currentChatHistoryId, setCurrentHistoryId] = useState<
    number | undefined
  >();
    ...
    fetch(import.meta.env.VITE_BASE_API_URL + "/chat/chat-history/get", {
      ...
      .then(async (data) => {
        ...
        try {
          ...
          toast.error(
            "Failed to load laws data. You may need to select laws again" +
              error.message
          );
        }
      });
  ...
  const handleDeleteChatHistory = async (history: ChatHistory) => {
    fetch(import.meta.env.VITE_BASE_API_URL + "/chat/chat-history/delete", {
      ...
      .catch((error: any) => {
        toast.error(
          "Terjadi kesalahan saat menghapus riwayat chat:",
          error.message
        );
      });
  };
    ...
    if (input.trim()) {
      ...
      try {
        const response = await fetch(
          import.meta.env.VITE_BASE_API_URL + "/chat/ask-chatbot",
          {
            method: "POST",
            body: JSON.stringify({
        ...
        if (response.ok) {
          ...
          await updateChatHistoryinDatabase([
            ...oldMessages,
            humanMessage,
            aiMessage,
          ]);
    ...
    <div className="h-screen flex overflow-y-scroll bg-gray-100">
        ...
        <div className="fixed inset-0 flex items-center justify-center p-6 lg:p-12 h-screen w-screen">
          <div className="relative w-full h-full">
            ...
            <DialogPanel className="h-full rounded-lg outline outline-1 shadow-2xl shadow-gray-600 bg-ch-almost-white">
              <ChooseLawsComponent
                setIsSelectLawsDialogOpen={setIsSelectLawsDialogOpen}
                isSelectLawsDialogOpen={isSelectLawsDialogOpen}
              />
            </DialogPanel>
          </div>
        </div>
        ...
        <div className="flex-1 px-4 overflow-y-scroll">
          ...
          {chatHistories.map((history, index) => (
            <div className="rounded-md w-full flex " key={index}>
              ...
              <button
                className="p-2  hover:bg-ch-brick-red rounded-md transition-all"
                onClick={() => handleDeleteChatHistory(history)}
              >
                <HiTrash size={25} />
              </button>
            </div>
      ...
      <div className="flex flex-col flex-1 h-screen">
        ...
        <header className=" bg-ch-coral h-16 px-4 text-white text-center flex">
          ...
          <a
            href="/#"
            className="text-2xl font-semibold my-auto hover:text-ch-grey transition-all"
          >
            ChatHukum.ai
          </a>
          <div className="flex-1" />
        ...
        <div
          id="menu"
          className={`fixed right-0 top-16 bg-ch-coral rounded-b-md text-left text-white shadow-md w-64 z-10 ${
            isMenuOpen ? "" : "hidden"
          }`}
        >
          <button
            onClick={() => setIsSelectLawsDialogOpen(true)}
            className="w-full px-4 text-left transition-all hover:bg-ch-brick-red  py-2"
          >
            Edit Daftar Dokumen
          </button>
          <button
        ...
        <div className="flex-1 p-4 overflow-y-auto">
          ...
          (messages.length > 0 || isLoading) ? (
                ...
                <div className="flex justify-start items-center">
                  <div className="w-64 flex px-4 py-2 rounded-lg  text-wrap bg-gray-300">
                    <p className="w-full py-2">
                      <Skeleton baseColor="#A9A9A9" count={5} />
                    </p>
                  </div>
                </div>
              )}
            ...
            <div className="flex w-full h-full flex-col justify-center items-center">
              <p className="text-gray-800 text-center max-w-md text-xl">
                Selamat datang di ChatHukum.ai! Anda dapat bertanya hal apapun
                terkait 250.000+ dokumen peraturan perundang-undangan yang ada
                di Indonesia.
              </p>
              {currentSelectedLaws.length && !messages.length ? (
                <p className="text-gray-800 text-center max-w-md text-xl mt-4">
                  Tulis pesan di bawah untuk memulai obrolan.
                </p>
              ) : (
                <></>
              )}
              {!currentSelectedLaws.length && (
                <p className="text-gray-800 text-center max-w-md text-xl mt-4">
                  Sebelum memulai, pilihlah terlebih dahulu dokumen peraturan
                  atau undang-undang yang akan ditanyakan.
                </p>
              )}
              {!currentSelectedLaws.length && (
                <button
                  className="flex bg-ch-coral hover:bg-ch-brick-red py-2 px-4 rounded-md mt-4"
                  onClick={() => setIsSelectLawsDialogOpen(true)}
                >
